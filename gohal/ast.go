...
func BuildAst(fileLines []string) []HalNode {
	...
	for idx, instruction := range fileLines {
		...
		case idx == 0:
			if startprogramRegex.MatchString(instruction) {
				ast[idx] = HalNode{Instruction: programStart, n: 0, loopStart: 0}
			} else {
				panic(ErrHalReallyConfused("program must start with 'Good afternoon, gentlemen.' command"))
			}
		case idx == (programLength-1) && instruction != "Stop, Dave.":
			panic(ErrHalReallyConfused("program must end with 'Stop, Dave.' command"))
		case instruction == "Stop, Dave.":
			ast[idx] = HalNode{Instruction: programEnd, n: 0, loopStart: 0}
		case strings.Contains(instruction, "Hal?"):
			ast[idx] =  HalNode{Instruction: incrementCell, n: strings.Count(instruction, "Hal!"), loopStart: 0}
		case strings.Contains(instruction, "I'm afraid. I'm afraid, Dave. Dave, my mind is going."):
			ast[idx] = HalNode{Instruction: decrementCell, n: strings.Count(instruction, "I can feel it."), loopStart: 0}
		case instruction ==  "What are you doing, Dave?":
			currentLoopId++
			loopStartIndexes[currentLoopId] = idx

			ast[idx] = HalNode{Instruction: loopStart, n: 0, loopStart: 0}
		case instruction == "Dave, this conversation can serve no purpose anymore. Goodbye.":
			loopStartIdx, _ := loopStartIndexes[currentLoopId]
			ast[idx] = HalNode{Instruction: loopEnd, n: 0, loopStart: loopStartIdx}
			currentLoopId--
		case instruction == "This mission is too important for me to allow you to jeopardize it.":
			ast[idx] = HalNode{Instruction: loopBreak, n: 0, loopStart: 0}
		case instruction == "I know I've made some very poor decisions recently, but I can give you my complete assurance that my work will be back to normal.":
			ast[idx] = HalNode{Instruction: loopBreakAll, n: 0, loopStart: 0}
		case instruction == "I've picked up a fault in the AE-35 unit.":
			ast[idx] = HalNode{Instruction: shiftLeft, n: 0, loopStart: 0}
		case instruction == "Well, he acts like he has genuine emotions.":
			ast[idx] = HalNode{Instruction: shiftRight, n: 0, loopStart: 0}
		case instruction == "Open the pod bay doors, HAL.":
			ast[idx] = HalNode{Instruction: userInput, n: 0, loopStart: 0}
		case instruction == "Close the pod bay doors, HAL.":
			ast[idx] = HalNode{Instruction: displayChar, n: 0, loopStart: 0}
		}
	}
